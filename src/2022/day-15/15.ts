...
class Solution {
    ...
    getDistance(location: Location, target: Location) {
        return (
            Math.abs(location.x - target.x) + Math.abs(location.y - target.y)
        );
    }
    ...
    getFreeSpot(bounds: Bounds) {
        ...
        for (let y = bounds.y.min; y <= bounds.y.max; y++) {
            for (let x = bounds.x.min; x <= bounds.x.max; x++) {
                ...
                if (
                    sensors.every(
                        (sensor) => !this.isInSensorsRange(sensor, location),
                    )
                ) {
                    return location;
                }
...
console.log(
    beaconExclusionZone(
        await Deno.readTextFile("./src/2022/day-15/input-15.txt"),
    ),
);
