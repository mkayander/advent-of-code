...
class RegolithReservoir {
  ...
  constructor(input: string, readonly pouringSpot: Location) {
  ...
  public generateRocks() {
    for (const rock of this.rocks) {
      for (let i = 1; i < rock.length; i++) {
        ...
        const direction = this.getDistanceVector(currentCorner, nextCorner);
...
console.log(
  regolithReservoir(await Deno.readTextFile('./src/2022/day-14/input-14.txt'))
);
